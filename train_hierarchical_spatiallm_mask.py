#!/usr/bin/env python3
"""Run hierarchical SFT with GT-bbox point-token masking in stage 2."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stage2_point_token_bbox_expand_ratio",
        "--stage2-point-token-bbox-expand-ratio",
        type=float,
        default=None,
        help=(
            "Override stage-2 point-token bbox mask expansion ratio. "
            "0.1 expands each side by 10%, making each dimension 1.2x."
        ),
    )
...
def main() -> None:
    ...
    stage1_config["point_token_bbox_mask"] = False
    stage2_config["point_token_bbox_mask"] = True
    if "point_token_bbox_expand_ratio" not in stage2_config:
        stage2_config["point_token_bbox_expand_ratio"] = 0.1
    apply_optional_float_override(
        stage2_config,
        "point_token_bbox_expand_ratio",
        args.stage2_point_token_bbox_expand_ratio,
    )
